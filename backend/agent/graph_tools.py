    ...
    try:
        ...
        for r in search_response.results:
            if r.node_type == NodeType.ENTITY:
                ...
                ts = r.content.get("timestamp") or r.content.get("start_time", 0)
                moments.append({
                    "timestamp": ts,
                    "timestamp_formatted": format_timestamp(ts),
...
async def find_highlights(
    ...
    try:
        ...
        # Helper to check if time range overlaps with existing
        ...
        # First, get video duration for distributing highlights
        video_duration = 0
        with kg.get_session() as session:
            result = session.run(
                """
        ...
        # Strategy 1: Get frames with rich descriptions from across the video
        # Divide video into segments and pick best from each segment
        num_segments = max(max_clips * 2, 10)  # More segments than clips needed
        ...
        for segment_idx, frame in sorted_segments:
            ...
            highlights.append({
                ...
                "highlight_reason": "Rich visual content",
                "suggested_for": ["social_clip", "preview"],
            })
            used_ranges.append((start, end))

        # Strategy 2: If we still need more, get scenes with visual variety
        if len(highlights) < max_clips:
            with kg.get_session() as session:
        ...
        # Strategy 3: Get frames with rich descriptions (interesting content)
        if len(highlights) < max_clips:
            with kg.get_session() as session:
                result = session.run(
                    """
            ...
            for frame in rich_frames:
                ...
                highlights.append({
                    ...
                    "description": desc[:200],
                    "highlight_reason": "Rich visual content",
                    "suggested_for": ["thumbnail", "summary"],
                })
                used_ranges.append((start, end))
        ...
        # Strategy 3: Get moments with key entities mentioned
        if len(highlights) < max_clips:
            with kg.get_session() as session:
                result = session.run(
                    """
